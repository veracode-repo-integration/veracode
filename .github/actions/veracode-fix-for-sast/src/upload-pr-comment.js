...
function generateDefaultCommentBody(prResponseStr) {
  try {
    ...
    return `## Veracode Fix for SAST - Pull Request Created
**PR:** ${prResponse.html_url || 'N/A'}

This PR contains automated fixes for SAST vulnerabilities.

### Next Steps:
1. Review the changes in the Fix for SAST PR.
2. Verify that tests pass.
3. Merge the PR to apply the fixes.
4. Re-run the SAST scan to verify the vulnerabilities are resolved.`;
  } catch (error) {
    return 'A pull request has been created with automated fixes for Veracode SAST vulnerabilities. Please review the changes.';
  }
}
