...
async function runFixSast(workspaceDir, actionPath, fixSastParams) {
  try {
    ...
    // Add fix parameters if specific issues
    if (fixSastParams && fixSastParams.trim() && fixSastParams !== 'SAST-*') {
      core.info(`Fix SAST params: ${fixSastParams}`);
      args.push('-i', fixSastParams);
    }

    // Run veracode fix sast command
